...
const Footer = () => {
  ...
  useEffect(() => {
    ...
    fetchContacts(language)
      .then((items) => { if (!cancelled) setContactItems(items); })
      .catch(() => { if (!cancelled) setContactItems([]); });
            ...
            <ul className="space-y-3 text-gray-400 text-sm md:text-base">
              {contactItems.map((item) => (
                <li key={item.id} className="flex items-start space-x-2">
                  <span className="text-primary mt-0.5 flex-shrink-0">{getContactIcon(item.type)}</span>
                  {item.link ? (
                    <a
                      href={item.link}
                      target={item.link.startsWith('http') ? '_blank' : undefined}
                      rel={item.link.startsWith('http') ? 'noopener noreferrer' : undefined}
                      className="hover:text-primary transition-colors duration-200 break-words"
                    >
                      {item.value}
                    </a>
                  ) : (
